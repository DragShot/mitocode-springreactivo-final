...
public class UsuarioServiceImpl extends CRUDImpl<Usuario, String> implements IUsuarioService{
	...
	@Override
	protected IRepo<Usuario, String> getRepo() {
		return repo; 
	}
	...
	public Mono<User> buscarPorUsuario(String usuario) {
		...
		return monoUsuario.flatMap(u -> {
			return Flux.fromIterable(u.getRoles())
					.flatMap(rol -> rolRepo.findById(rol.getId()).map(r -> { roles.add(r.getNombre()); return r; }))
					.collectList().flatMap(list -> { u.setRoles(list); return Mono.just(u); });
		})	
		.map(u -> new User(u.getUsuario(), u.getClave(), u.getEstado(), roles));
	}
}
